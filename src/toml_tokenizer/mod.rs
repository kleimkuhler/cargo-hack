...
#[derive(Debug, Clone)]
pub(crate) struct TomlTokenizer {
    was_sorted: bool,
    pub(crate) tables: Vec<TomlTable>,
    inner: TomlString,
}
...
impl TomlTokenizer {
    fn new() -> Self {
        Self { was_sorted: false, tables: Vec::default(), inner: TomlString::default() }
    }

    /// Clone only the tables
    pub(crate) fn clone_tables(&self) -> Vec<TomlTable> {
        self.tables.clone()
    }
    ...
    pub(crate) fn sort_nested(&mut self, field: &str) {
    ...
    /// ```
    pub(crate) fn sort_items(&mut self, key: &str) {
    ...
    pub(crate) fn was_sorted(&self) -> bool {
        self.was_sorted
    }

    pub(crate) fn iter(&self) -> TokenIter<'_> {
        TokenIter { inner: self, idx: 0 }
    }

    pub(crate) fn iter_mut(&mut self) -> impl Iterator<Item = &mut TomlTable> {
    ...
    fn parse(s: &str) -> Result<Self::Item, Self::Error> {
        ...
        while tokenizer.inner.has_more() {
            ...
            if !end {
                ...
                let table = TomlTable { header: Some(header), items: Some(items), comment };
                tokenizer.tables.push(table);
            // println!("{:#?}", items);
            } else {
                let table = TomlTable { header: None, items: None, comment };
                tokenizer.tables.push(table);
            }
        }
        Ok(tokenizer)
...
impl fmt::Display for TomlTokenizer {
...
pub(crate) struct FilterTake<'a, P> {
    predicate: P,
    idx: usize,
    steal_idx: usize,
    old_len: usize,
